...
| M-c             | copy                                                    |
| M-v             | paste                                                   |
| C-/             | comment or uncomment (doesn't work in terminal somehow) |
...
#+begin_src emacs-lisp
  ...
  (use-package ivy
    ...
    (ivy-mode 1)
    :bind
    (("C-q" . swiper-isearch) ; Don't need reverse search in swiper. Can just cycle back.
...
#+begin_src emacs-lisp
  ...
  (global-set-key (kbd "C-i") 'kill-region) ; Because `C-w` is remapped to backward kill word.
  ...
  ;; Aliases.
  (defalias 'reload  'my/reload-init-file)
  (defalias 'config  'my/edit-configs)
...
(use-package org
  :ensure nil
  :defer t
  ; Unbind `C-i` to avoid overwriting for the globally set key that mapped to `kill-region`.
...
#+begin_src emacs-lisp
  (use-package undo-tree
    ...
    ; Unbind `C-/` to avoid overwriting for the globally set key mapped to comment.
