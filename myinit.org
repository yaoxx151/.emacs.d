...
Commonly used commands should be binded to short, easy-to-reach keybindings:
| Key   | Command                         |
|-------+---------------------------------|
| C-s   | Save                            |
| C-o   | Switch window                   |
...
Keybindings that end with `f` and `b` are always awkward to me since they're
hard to reach even though they remind you it's forward and backward. Rebind for
more symmetry:
| Key   | Command                         |
|-------+---------------------------------|
| M-j   | Backward word                   |
...
Since the original C-b, C-f, M-b, M-f are free now, we can rebind them to
common commands:
| Key   | Command                   |
|-------+---------------------------|
...
#+begin_src emacs-lisp
  ...
  (use-package ivy
    ...
    (("C-f" . counsel-M-x)        ; Because C-f is freed, why not?
     ("M-f" . counsel-find-file)  ; Because M-f is freed, why not?
     ("M-b" . ivy-switch-buffer)  ; Because M-b is freed, why not?
     ("C-q" . swiper-isearch)
...
#+begin_src emacs-lisp
  ...
  ;; Useless keys.
  (global-unset-key (kbd "C-b"))      ; Original: backward char.
  (global-unset-key (kbd "C-x b"))    ; Original: switch buffer.
...
#+begin_src emacs-lisp
  ...
  ;; C-g is always hard to press but esc is also far away.
  ...
  ;; Bind extra keys to common commands around to reduce mispressing.
