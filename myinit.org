...
| `C-x C-m` | M-x (thanks, Stevey. Same for `C-w`.)      |
...
#+end_src
* Company
Only some basic code completion. (Is that even too much?) No heavy LSP stuffs.
#+begin_src emacs-lisp
  (use-package company
    :ensure t
    :config
...
#+begin_src emacs-lisp
  (use-package crux
    ...
    (("C-x K" . crux-kill-other-buffers)
     ...
     ("C-j" . crux-smart-kill-line) ; In case for mispress.
...
Why does Emacs put those garbage by default that take screen space?
#+begin_src emacs-lisp
  ...
  ;; Useless keys.
  (global-unset-key (kbd "C-z"))     ; Don't hit it and suspend it in terminal. Stupid key.
...
#+end_src
* Dumb Jump
Useful to go to code definitions. Old packages that might not work in the future.
Why doesn't Emacs have better go to definition packages/commands? It should be a default feature.
#+begin_src emacs-lisp
...
#+end_src
* Goto-chg
Old packages might not work in the future too. Such a basic function that should be default.
#+begin_src emacs-lisp
...
#+end_src
* Magit
Make git slightly less painful, but git is still a pain in ass.
#+begin_src emacs-lisp
  (use-package magit
    :ensure t
    :bind
...
#+end_src
* Multi-cursor
Useful but can be tempting to show off and waste time.
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  ...
  (defun my/go-to-middle-of-line ()
    "Move the cursor to the middle of the current line."
    (interactive)
    (let ((middle (floor (/ (line-end-position) 2))))
      (goto-char middle)))

  (global-set-key (kbd "M-l") 'my/go-to-middle-of-line)
...
#+begin_src emacs-lisp
  ...
  ;; Don't show useless info in mode line.
...
#+end_src
* Org
Org is great as long as you don't spend your life on it exploring all the features.
#+begin_src emacs-lisp
  (use-package org
    :ensure nil
    :defer t
...
#+end_src
* Projectile
A hack that shouldn't exist if GUI Emacs can isolate buffers in each instance.
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  ...
  (use-package view
    ...
    (global-set-key (kbd "C-t") 'View-scroll-half-page-backward))
...
#+begin_src emacs-lisp
  (use-package zenburn-theme
    ...
    ;; Mode line.
    (set-face-attribute 'mode-line-buffer-id nil :foreground "gray40" :underline)
