...
#+begin_src emacs-lisp
  (use-package avy
    ...
    :defer t
    :bind
    ("M-m" . avy-goto-char-timer) ; Just a key easy to press.
    :custom
...
Why does Emacs put those garbage by default?
#+begin_src emacs-lisp
...
#+end_src
* Goto-chg
Old package; might not work in the future too. Such a basic function that should be default.
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  ...
  ;; Default `M-b` and `M-f` are always awkward to me.
...
#+end_src
* Org
Org is great as long as you don't spend your whole life on it.
#+begin_src emacs-lisp
  (use-package org
    :ensure nil
    :defer t
...
#+end_src
* Projectile
Shouldn't be used if GUI Emacs can isolate buffers in each instance.
#+begin_src emacs-lisp
