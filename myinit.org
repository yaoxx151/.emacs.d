...
#+begin_src emacs-lisp
  ...
  (crux-with-region-or-line comment-or-uncomment-region) ; `M-;` works on line.
...
#+begin_src emacs-lisp
  ...
  (remove-hook 'org-mode-hook 'electric-indent-mode) ; Cause annoying auto indent in org mode.
